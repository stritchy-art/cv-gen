...
def display_rate_calculator(cv_data=None):
    ...
    st.markdown("<hr style='border: 3px solid #1D435B; margin: 20px 0;'>", unsafe_allow_html=True)
    ...
    if cv_data:
        ...
        if not suggested_rate or suggested_rate == 0:
            ...
            years_exp = extract_years_experience(cv_data)
            st.info(f"🤖 TJM suggéré par l'IA (basé sur {years_exp} ans d'expérience) : **{suggested_rate} €/jour**")
        
        # Mettre à jour le TJM dans session_state
        if 'suggested_tjm' not in st.session_state or st.session_state.get('suggested_tjm') != suggested_rate:
    ...
    if st.button(
        ...
        calculated_cjm = cjm
        if sab > 0:
            calculated_cjm = ((sab / settings.WORKING_DAYS_PER_YEAR) * settings.MARKUP_COEFFICIENT) + settings.FIXED_COSTS
