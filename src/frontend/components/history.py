...
def get_cv_from_history(
    pdf_filename: str, options: Optional[dict] = None
) -> Optional[Dict]:
    """
    ...
    with st.sidebar:
        ...
        if not history:
            ...
            with st.expander(t("history_debug"), expanded=False):
                ...
                st.write(
                    f"{t('history_total_keys')} {len(list(history_cache.iterkeys()))}"
                )
            return None
        ...
        for idx, entry in enumerate(history):
            ...
            options_info = []
            if (
                options.get("improvement_mode")
                and options["improvement_mode"] != "none"
            ):
            ...
            with st.expander(expander_label, expanded=False):
                ...
                if st.button(
                    t("history_regenerate"), key=f"reload_{idx}_{pdf_filename}"
                ):
                    return pdf_filename
