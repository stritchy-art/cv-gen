...
def process_conversion(uploaded_files, improvement_mode, job_offer_file, generate_pitch, api_url, candidate_name=None, max_pages=None, target_language='fr'):
    ...
    try:
        ...
        for file_index, uploaded_file in enumerate(uploaded_files, 1):
            ...
            status_text.text(t("processing_cv", current=file_index, total=total_files, filename=uploaded_file.name))
            ...
            try:
                ...
                cache_options = {
                    ...
                    'job_offer_content': job_offer_file.name if job_offer_file else None,
                ...
                if cached_entry:
                    ...
                    app_logger.info(f"CV {uploaded_file.name} trouvé en cache avec ces options")
                ...
                if response.status_code == 200:
                    ...
                    if result.get('cv_data'):
                        try:
                            ...
                            app_logger.info(f"CV {uploaded_file.name} sauvegardé dans l'historique")
                        except Exception as e:
                            app_logger.error(f"Erreur sauvegarde historique: {e}")
                    else:
                        app_logger.warning(f"Pas de cv_data pour {uploaded_file.name}, historique non sauvegardé")
                    
                    # Stocker les résultats avec le contenu binaire au lieu de l'objet Response
                    all_results.append({
                        'filename': uploaded_file.name,
                        'result': result,
                        'docx_content': download_response.content if download_response.status_code == 200 else None,
                ...
                app_logger.error(f"Erreur traitement {uploaded_file.name}: {str(e)}", exc_info=True)
