...
async def convert_cv(
    ...
    improvement_mode: str = Form("none", description=t("improvement_mode_description", lang="fr")),
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=t("error_invalid_improvement_mode", lang="fr", values=', '.join([m.value for m in ImprovementMode]))
        )
    ...
    if job_offer_file:
        ...
        if not any(job_offer_file.filename.lower().endswith(ext) for ext in allowed_extensions):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
    ...
    try:
        api_logger.info(f"Requête de conversion reçue: {file.filename} (mode: {improvement_mode})")
        ...
        if max_pages:
            try:
                ...
                api_logger.info(f"Limitation de pages activée: {max_pages_int} page(s) maximum")
            except ValueError:
        ...
        if target_language:
            ...
            if target_language not in valid_languages:
                api_logger.warning(f"Langue cible invalide: {target_language}, défaut à None")
...
async def convert_and_download_cv(
    ...
    improvement_mode: str = Form("none", description=t("improvement_mode_description", lang="fr")),
    job_offer_file: Optional[UploadFile] = File(None, description=t("job_offer_targeted_description", lang="fr"))
):
    """
    ...
    try:
        api_logger.info(f"Requête de conversion+téléchargement: {file.filename} (mode: {improvement_mode})")
...
async def download_from_cache(conversion_id: str):
    ...
    now = datetime.now()
    expired = [k for k, v in conversion_cache.items() 
    ...
    if not cached:
        raise HTTPException(status_code=404, detail=t("error_conversion_expired", lang="fr"))
    
    docx_path = cached['docx_path']
    
    if not Path(docx_path).exists():
        raise HTTPException(status_code=404, detail=t("error_file_not_found", lang="fr"))
