...
class CVConversionService:
    ...
    def convert_pdf_to_docx(
        ...
        try:
            ...
            if generate_pitch:
                if pitch:
                    self.logger.info(f"Pitch généré avec succès: {len(pitch)} caractères")
                else:
                    self.logger.warning("Le pitch n'a pas été généré malgré generate_pitch=True")
            
            processing_time = time.time() - start_time
            
            self.logger.info(
                f"Conversion réussie: {output_file} "
                f"(durée: {processing_time:.2f}s)"
            )
