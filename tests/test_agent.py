...
class TestCVConverterAgent:
    ...
    def test_generate_cache_key_with_job_offer(self):
        ...
        with patch.dict(os.environ, {'OPENAI_API_KEY': 'test-key'}):
            agent = CVConverterAgent()
            key1 = agent._generate_cache_key("test content", True, "targeted", "job offer")
    ...
    def test_extract_structured_data_with_llm_basic(self, mock_openai_class, mock_cache):
        ...
        with patch.dict(os.environ, {'OPENAI_API_KEY': 'test-key'}):
            ...
            result = agent.extract_structured_data_with_llm("CV content", model="gpt-4o-mini")
    ...
    @patch('core.agent.llm_cache')
    @patch('core.agent.OpenAI')
    def test_extract_structured_data_with_llm_improvement(self, mock_openai_class, mock_cache):
    ...
    @patch('core.agent.llm_cache')
    @patch('core.agent.OpenAI')
    def test_extract_structured_data_with_llm_targeted(self, mock_openai_class, mock_cache):
    ...
    @patch('core.agent.OpenAI')
    @patch('core.agent.llm_cache')
    def test_extract_structured_data_with_translation(self, mock_cache, mock_openai_class):
        ...
        with patch.dict(os.environ, {'OPENAI_API_KEY': 'test-key'}):
            ...
            mock_response.choices = [Mock()]
            mock_response.choices[0].message.content = json.dumps({
                "header": {"name": "Test", "title": "Developer", "experience": "5 years"},
                "competences": {},
                "formations": [],
                "experiences": []
            })
    ...
    @patch('core.agent.OpenAI')
    @patch('core.agent.llm_cache')
    def test_extract_structured_data_with_max_pages(self, mock_cache, mock_openai_class):
    ...
    def test_generate_profile_pitch_with_job_offer(self, mock_openai_class, mock_cache):
        ...
        with patch.dict(os.environ, {'OPENAI_API_KEY': 'test-key'}):
            ...
            call_args = agent.client.chat.completions.create.call_args
            assert "Recherche développeur Python" in call_args[1]["messages"][1]["content"]
    ...
    def test_process_cv_pdf_basic(self, mock_gen_docx, mock_extract_llm, mock_extract_pdf):
        ...
        with patch.dict(os.environ, {'OPENAI_API_KEY': 'test-key'}):
            ...
            try:
                # Mocks
                mock_extract_pdf.return_value = "PDF text content with sufficient length " * 10
    ...
    def test_process_cv_docx(self, mock_gen_docx, mock_extract_llm, mock_extract_docx):
        ...
        with patch.dict(os.environ, {'OPENAI_API_KEY': 'test-key'}):
            ...
            try:
                # Mocks
                mock_extract_docx.return_value = "DOCX text content with sufficient length " * 10
                mock_extract_llm.return_value = {
                    "header": {"name": "Test User", "title": "Dev", "experience": "5 ans"},
                    "competences": {},
                    "formations": [],
    ...
    def test_process_cv_with_pitch(self, mock_pitch, mock_gen_docx, mock_extract_llm, mock_extract_pdf):
    ...
    def test_process_cv_insufficient_content(self, mock_extract_pdf):
        ...
        with patch.dict(os.environ, {'OPENAI_API_KEY': 'test-key'}):
            ...
            try:
                ...
                with pytest.raises(ValueError, match="contenu extrait du CV est insuffisant"):
    ...
    @patch('core.agent.CVConverterAgent.extract_structured_data_with_llm')
    @patch('core.agent.generate_docx_from_cv_data')
    def test_process_cv_with_candidate_name(self, mock_gen_docx, mock_extract_llm, mock_extract_pdf):
        ...
        with patch.dict(os.environ, {'OPENAI_API_KEY': 'test-key'}):
            ...
            try:
                mock_extract_pdf.return_value = "PDF text content with sufficient length " * 10
                mock_extract_llm.return_value = {
                    "header": {"name": "Original Name", "title": "Dev", "experience": "5 ans"},
                    "competences": {},
                    "formations": [],
                    "experiences": []
                }
