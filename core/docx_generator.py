...
class CVDocxGenerator:
    ...
    def __init__(self, target_language='fr'):
        self.doc = Document()
        self.target_language = target_language if target_language in self.LABELS else 'fr'
    ...
    def _add_page_footer(self):
        ...
        for row in table.rows:
            for cell in row.cells:
                cell._element.get_or_add_tcPr().append(
                    parse_xml(r'<w:tcBorders %s><w:top w:val="none"/><w:left w:val="none"/><w:bottom w:val="none"/><w:right w:val="none"/></w:tcBorders>' % nsdecls('w'))
                )
        ...
        # Copyright
        run = right_para.add_run("© ALLTECH - Tous droits réservés / CONFIDENTIEL ALLTECH")
        run.font.size = Pt(9)
        run.font.color.rgb = self.COLOR_GRAY
        run.bold = True
    ...
    def _add_page_header(self, cv_data, output_path):
        ...
        table.columns[1].width = Inches(6.0) # Fiche de compétences + Nom (centré, très large)
            ...
            for tech in competences['techniques']:
                if isinstance(tech, dict):
                    ...
                    if isinstance(items, list):
                        ...
                        items_str = str(items)
                    tech_items.append(f"{category} : {items_str}" if category else items_str)
                else:
    ...
    def _add_formations(self, cv_data):
        ...
        for formation in formations:
            if isinstance(formation, dict):
                ...
                formation_items.append(f"{year} : {description}" if year else description)
            else:
